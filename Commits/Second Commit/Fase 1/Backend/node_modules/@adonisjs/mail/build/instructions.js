...
async function instructions(projectRoot, app, sink) {
    ...
    const mailContract = new sink.files.MustacheFile(projectRoot, contractsPath, getStub('contract.txt'));
    ...
    if (mailDrivers.includes('ses')) {
        ...
        const spinner = sink.logger.await(`Installing packages: ${pkg.getInstalls(false).list.join(', ')}`);
        try {
