...
class Supercharged {
    ...
    discoverComponents(basePath, options = {}) {
        const components = utils_1.fsReadAll(path_1.join(basePath, 'components'), (file) => file.endsWith('.edge'));
        components.forEach((file) => {
            file = slash_1.default(file);
            const name = file
                .replace(/\.edge$/, '') // Drop extension
                .split('/') // Split path
                .map((segment) => camelcase_1.default(segment)) // Convert each segment to camelCase
                .join('.'); // Join by .
            /**
